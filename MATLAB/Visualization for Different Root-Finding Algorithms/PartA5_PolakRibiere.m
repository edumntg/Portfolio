...
G_new = [2*x(1,k) - 400*x(1,k)*(-x(1,k)^2 + x(2,k)) - 2;
...
while norm(G_new) > tol
    if k == 1
        d(:, k) = -G_new;
    else
        d(:, k) = -G_new + (G_new'*(G_new - G_old))/norm(G_old)^2 *d(:,k-1);
    end
    j = 1;
    k2 = 1;
    while(j > 0)
        if f(x(1,k)+ alpha*d(1,k),x(2,k) + alpha*d(2,k)) <= f(x(1,k), x(2,k)) + gamma*alpha*G_new'*d(:,k)
            j = 0;
        else
            alpha= alpha*delta;
        end
    ...
    G_new = [2*x(1,k+1) - 400*x(1,k+1)*(-x(1,k+1)^2 + x(2,k+1)) - 2;
...
xlabel('K');
ylabel('Jk');
title('Cost for Polak-Ribiere Method')
